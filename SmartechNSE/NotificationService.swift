...
class NotificationService: UNNotificationServiceExtension {
  ...
  override func didReceive(_ request: UNNotificationRequest, withContentHandler handler: @escaping (UNNotificationContent) -> Void) {
    ...
    print("[APPBOY] Push with mutable content received.")
      ...
      if SmartPush.sharedInstance().isNotification(fromSmartech: request.content.userInfo) {
          smartechServiceExtension.didReceive(request, withContentHandler: contentHandler ?? { _ in })
          return
      }
  ...
  override func serviceExtensionTimeWillExpire() {
    ...
    print("[APPBOY] serviceExtensionTimeWillExpire, abortOnAttachmentFailure: \(abortOnAttachmentFailure)")
    
    if abortOnAttachmentFailure {
      print("[APPBOY] Time has expired and the service will terminate.")
      return
    }
      
    displayOriginalContent("serviceExtensionTimeWillExpire")
  }
}
